...
class MakeDataSet:
    def __init__(self, LIDC_Patients_list, IMAGE_DIR, MASK_DIR,CLEAN_DIR_IMAGE,CLEAN_DIR_MASK,META_DIR, mask_threshold, padding, confidence_level=0.5):
        ...
        self.meta = pd.DataFrame(index=[], columns=['patient_id', 'nodule_no', 'slice_no', 'original_image', 'mask_image', 'malignancy', 'subtlety', 'texture', 'sphericity', 'margin', 'is_cancer', 'is_clean'])
    ...
    # Extract characteristics like subtlety, texture, sphericity, margin
    ...
    def save_meta(self,meta_list):
        ...
        tmp = pd.Series(meta_list, index=['patient_id', 'nodule_no', 'slice_no', 'original_image', 'mask_image', 'malignancy', 'subtlety', 'texture', 'sphericity', 'margin', 'is_cancer', 'is_clean'])
        ...
        for patient in tqdm(self.IDRI_list):
            ...
            if len(nodules_annotation) > 0:
                ...
                # This current for loop iterates over total number of nodules in a single patient
                    ...
                    # Check if the mask is empty - added code.
                    if lung_np_array.shape[2] == 0:
                        print(f"Skipping nodule {nodule_idx} for patient {pid} due to empty mask.")
                        continue
                    ...
                    for nodule_slice in range(mask.shape[2]):
                        ...
                        nodule_name_for_meta = "{}/{}".format(pid, nodule_name)
                        mask_name_for_meta = "{}/{}".format(pid, mask_name)
                        meta_list = [pid[-4:],nodule_idx,prefix[nodule_slice],nodule_name_for_meta,mask_name_for_meta,malignancy, subtlety, texture, sphericity, margin, cancer_label,False]
                ...
                for slice in range(vol.shape[2]):
                    ...
                    nodule_name_for_meta = "{}/{}".format(pid, nodule_name)
                    mask_name_for_meta = "{}/{}".format(pid, mask_name)
                    meta_list = [pid[-4:],slice,prefix[slice],nodule_name_for_meta,mask_name_for_meta,0, subtlety, texture, sphericity, margin, False,True]
                    self.save_meta(meta_list)
...
if __name__ == '__main__':
    ...
    # Modified code - bottom 2 lines.
    # Find the index of the starting patient
    start_index = LIDC_IDRI_list.index('LIDC-IDRI-0001')

    # Slice the list to start from LIDC-IDRI-0455
